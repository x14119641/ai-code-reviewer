...
## Maintainability Verification Experiment

The taxonomy-expansion and generalization experiments exposed a recurring
precision problem in the maintainability specialist.

The specialist can correctly identify structural maintainability issues,
but it can also produce findings when:

```text
the issue already existed before the diff
```

or when:

```text
a benchmark from another rule family
        ↓
contains code that superficially resembles
a maintainability problem
        ↓
specialist emits unrelated finding
```

Rather than immediately modifying `maintainability_v1`, an additional
experimental verification stage was introduced.
...
The verifier does not search for additional issues.
...
verify exact rule
        +
verify triggering condition
        +
verify diff attribution
        ↓
keep or reject
```

This means the verifier is intended primarily as a precision mechanism.

It cannot recover an issue that the specialist failed to generate.

## maintainability_verify_v1

The first verifier prompt was introduced as:

```text
maintainability_verify_v1
```

It verifies only the maintainability candidates generated by the
specialist.

The verifier explicitly checks:

```text
current code satisfies rule
+
diff introduced or worsened condition
+
condition was not already present
+
candidate uses exact rule
```

For `duplicate_code`, attribution requires at least one side of the
meaningful duplicated implementation to have been introduced or
materially changed by the diff.

For `long_function`, the verifier requires the diff itself to have
expanded a previously focused function into multiple distinct
responsibilities.

Initial targeted experiments showed that this additional stage could
remove some spurious maintainability findings.

However, it did not consistently solve `duplicate_code` attribution.

## Duplicate-Code Generalization Diagnostic

The existing held-out `duplicate_code` family contains three cases:
...
Model                              qwen3.5:9b
General prompt                     v13
Maintainability specialist         maintainability_v1
Maintainability verifier           maintainability_verify_v1
Context                            4096
```
...
The reported finding correctly recognizes that the current source
contains duplicated validation logic.

However, both duplicated implementations already existed before the
change.

The diff only adds an unrelated function.

Therefore:

```text
issue exists in AFTER
        ≠
issue introduced by diff
```

This is specifically a change-attribution failure.

## Verifier v2 Attribution Experiment

A second experimental verifier prompt was created:

```text
maintainability_verify_v2
```

The purpose was to strengthen attribution requirements rather than
change the underlying `duplicate_code` definition.

The verifier was explicitly instructed to:

```text
identify exact triggering code
        ↓
locate triggering code in diff
        ↓
prove triggering code was added or materially changed
        ↓
reject unchanged pre-existing implementation
```

For duplicate code specifically:

```text
at least one side of meaningful duplication
must be introduced or materially changed
```

and:

```text
both duplicated implementations unchanged
        +
unrelated diff elsewhere
        ↓
reject
```

Despite these stronger instructions, the targeted result regressed:
...
The stronger verifier therefore failed in both directions:

```text
valid introduced duplication
        ↓
rejected
...
Adding increasingly explicit attribution wording to the verifier does
not currently produce reliable duplicate-code attribution.

## Candidate-Stage Isolation

To determine whether the attribution problem originated only in the
verifier, the specialist candidate stage was evaluated independently.
...
on the introduced duplicate case produced:
...
The same candidate stage was then evaluated on the pre-existing
duplicate boundary.
...
The specialist therefore generates:
...
```text
pre-existing duplication + unrelated change
```

This isolates the underlying behavior more clearly.

The problem is not simply:

```text
verifier incorrectly modifies a good candidate set
```
...
specialist candidate generation
        ↓
already contains attribution false positive
        ↓
verifier must distinguish two
structurally very similar candidate findings
```

The current verifier prompts are not reliably making that distinction.

## Performance Regression Check

The verifier experiment was also checked against performance
generalization families that had previously exposed spurious
...
The previously observed unrelated maintainability interference does not
appear in this targeted verifier run.

### Repeated Expensive Call in Loop

Using the same architecture produced:
...
pre-existing repeated expensive operation
        ↓
PASS

introduced invariant repeated expensive operation
        ↓
PASS
```

This provides evidence that verification can improve specialist
precision on cases outside the maintainability domain.

However, the duplicate-code attribution failure remains unresolved.

## Verification Experiment Interpretation

The current evidence suggests that maintainability verification has
potential value, but it should not yet replace the established
architecture.

Positive evidence:

```text
list_membership_in_loop generalization
        ↓
2/2

repeated_expensive_call_in_loop generalization
        ↓
3/3
```

The verifier can suppress some unrelated specialist findings.
...
stronger verifier attribution wording
        ↓
regresses introduced positive
        +
does not fix pre-existing false positive
```

The experiment therefore reinforces an earlier architectural principle:

```text
more prompt instructions
        ≠
reliable reasoning improvement
```

The current evidence does not justify continued verifier-prompt tuning
against the same three duplicate-code examples.

## Architectural Consequence

The duplicate-code experiment exposes a useful separation between three
review responsibilities:

```text
DISCOVERY
Is there meaningful duplicated implementation?

ATTRIBUTION
Did this diff introduce or materially increase it?

VERIFICATION
Is the proposed finding sufficiently supported?
```

The current maintainability specialist performs discovery well:
...
but does not consistently perform attribution:
...
The verifier can improve precision in some cases, but the current prompt
experiments show that simply asking another LLM call to repeat
attribution reasoning is not sufficient.

The next architectural hypothesis should therefore investigate whether
change attribution deserves a more explicit stage or representation,
rather than continuing to enlarge the verifier prompt.
...
candidate discovery
        ↓
explicit BEFORE / AFTER attribution evidence
        ↓
rule verification
```

or deterministic preprocessing that exposes the relevant changed code
more directly to the attribution stage.

These are architectural hypotheses, not established improvements.

They should be evaluated against multiple attribution families before
being adopted.

## Current Verifier Status

The verifier remains experimental.

```text
maintainability_verify_v1
        ↓
promising precision behavior
        +
duplicate_code attribution unresolved

maintainability_verify_v2
        ↓
stronger attribution wording
        +
duplicate_code regression
        ↓
not preferred
```

The established expanded architecture therefore remains:
...
The verifier should not yet be considered part of the frozen baseline.

The experiment is retained because it provides evidence for the next
architectural investigation rather than because it produces a new
benchmark baseline.
...
6. Investigate maintainability architecture at the responsibility level:
       discovery
       attribution
       verification

   Determine whether attribution should:
       remain inside the specialist
       use a dedicated attribution stage
       use deterministic diff-derived evidence
       or combine deterministic evidence with LLM verification

7. Re-evaluate excessive_nesting ownership only after the
   maintainability architecture is clearer

8. Only then modify prompts if repeated evidence justifies it
```
